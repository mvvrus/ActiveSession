...
namespace ActiveSession.Tests
{
    public class ExtRunnerKeyTests
    {
        ...
        const String AS_ID_ENC = "382c74c3-721d-4f34-80e5-57657b6cbc27%22%3C";
        [Fact]
        public void ToStringTest()
        {
            ExtRunnerKey value = new ExtRunnerKey(RunnerNumber: 42, Generation: 19, ActiveSessionId: AS_ID);
            Assert.Equal($"42-19-{AS_ID_ENC}", value.ToString());
        }
        ...
        public void TryParseTest()
        {
            ...
            Assert.True(ExtRunnerKey.TryParse($"42-19-{AS_ID_ENC}", out value));
            Assert.Equal(sample, value);
            Assert.False(ExtRunnerKey.TryParse($" 42-19-{AS_ID_ENC}", out value));
            Assert.False(ExtRunnerKey.TryParse($"42-19 -{AS_ID_ENC}", out value));
            Assert.False(ExtRunnerKey.TryParse($"42:19-{AS_ID_ENC}", out value));
            Assert.False(ExtRunnerKey.TryParse("19-{AS_ID_ENC}", out value));
